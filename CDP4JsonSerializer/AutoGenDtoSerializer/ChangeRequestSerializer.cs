// -------------------------------------------------------------------------------------------------------------------------------
// <copyright file="ChangeRequestSerializer.cs" company="Starion Group S.A.">
...
namespace CDP4JsonSerializer
{
    ...
    using ChangeRequest = CDP4Common.DTO.ChangeRequest;
    ...
    public class ChangeRequestSerializer : BaseThingSerializer, IThingSerializer
    {
        ...
        /// <exception cref="ArgumentException">If the provided <paramref name="thing" /> is not an <see cref="ChangeRequest" /></exception>
        /// <exception cref="NotSupportedException">If the provided <paramref name="requestedDataModelVersion" /> is not supported</exception>
        public void Serialize(Thing thing, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            if (thing is not ChangeRequest changeRequest)
            {
                throw new ArgumentException("The thing shall be a ChangeRequest", nameof(thing));
            }

            if (requestedDataModelVersion < Version.Parse("1.1.0"))
            {
                Logger.Log(LogLevel.Info, "Skipping serialization of ChangeRequest since Version is below 1.1.0");
            ...
            {
                case "1.1.0":
                    Logger.Log(LogLevel.Trace, "Serializing ChangeRequest for Version 1.1.0");

                    //if (changeRequest.ApprovedBy.Count > 0)
                    //{
                        writer.WriteStartArray("approvedBy"u8);

                        foreach(var approvedByItem in changeRequest.ApprovedBy.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(changeRequest.Author);

                    //if (changeRequest.Category.Count > 0)
                    //{
                        writer.WriteStartArray("category"u8);

                        foreach(var categoryItem in changeRequest.Category.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(changeRequest.Classification.ToString());
                    writer.WritePropertyName("classKind"u8);
                    writer.WriteStringValue(changeRequest.ClassKind.ToString());
                    writer.WritePropertyName("content"u8);
                    writer.WriteStringValue(changeRequest.Content);
                    writer.WritePropertyName("createdOn"u8);
                    writer.WriteStringValue(changeRequest.CreatedOn.ToString(SerializerHelper.DateTimeFormat));

                    //if (changeRequest.Discussion.Count > 0)
                    //{
                        writer.WriteStartArray("discussion"u8);

                        foreach(var discussionItem in changeRequest.Discussion.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (changeRequest.ExcludedDomain.Count > 0)
                    //{
                        writer.WriteStartArray("excludedDomain"u8);

                        foreach(var excludedDomainItem in changeRequest.ExcludedDomain.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (changeRequest.ExcludedPerson.Count > 0)
                    //{
                        writer.WriteStartArray("excludedPerson"u8);

                        foreach(var excludedPersonItem in changeRequest.ExcludedPerson.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(changeRequest.Iid);
                    writer.WritePropertyName("languageCode"u8);
                    writer.WriteStringValue(changeRequest.LanguageCode);
                    writer.WritePropertyName("modifiedOn"u8);
                    writer.WriteStringValue(changeRequest.ModifiedOn.ToString(SerializerHelper.DateTimeFormat));
                    writer.WritePropertyName("owner"u8);
                    writer.WriteStringValue(changeRequest.Owner);
                    writer.WritePropertyName("primaryAnnotatedThing"u8);

                    if(changeRequest.PrimaryAnnotatedThing.HasValue)
                    {
                        writer.WriteStringValue(changeRequest.PrimaryAnnotatedThing.Value);
                    }
                    else
                    {
                    ...
                    //if (changeRequest.RelatedThing.Count > 0)
                    //{
                        writer.WriteStartArray("relatedThing"u8);

                        foreach(var relatedThingItem in changeRequest.RelatedThing.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteNumberValue(changeRequest.RevisionNumber);
                    writer.WritePropertyName("shortName"u8);
                    writer.WriteStringValue(changeRequest.ShortName);

                    //if (changeRequest.SourceAnnotation.Count > 0)
                    //{
                        writer.WriteStartArray("sourceAnnotation"u8);

                        foreach(var sourceAnnotationItem in changeRequest.SourceAnnotation.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(changeRequest.Status.ToString());
                    writer.WritePropertyName("title"u8);
                    writer.WriteStringValue(changeRequest.Title);
                    break;
                case "1.2.0":
                    Logger.Log(LogLevel.Trace, "Serializing ChangeRequest for Version 1.2.0");
                    ...
                    writer.WriteStringValue(changeRequest.ThingPreference);
                    writer.WritePropertyName("title"u8);
                    writer.WriteStringValue(changeRequest.Title);
                    break;
                case "1.3.0":
                    Logger.Log(LogLevel.Trace, "Serializing ChangeRequest for Version 1.3.0");
                    writer.WritePropertyName("actor"u8);

                    if(changeRequest.Actor.HasValue)
                    {
                        writer.WriteStringValue(changeRequest.Actor.Value);
                    }
                    else
                    {
                        writer.WriteNullValue();
        ...
        /// <summary>
        /// Serialize a value for a <see cref="ChangeRequest"/> property into a <see cref="Utf8JsonWriter" />
        /// </summary>
        ...
        public void SerializeProperty(string propertyName, object value, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            ...
            switch(propertyName.ToLower())
            {
                ...
                case "title":
                    ...
                    break;
                default:
                    throw new ArgumentException($"The requested property {propertyName} does not exist on the ChangeRequest");
            }
        }
