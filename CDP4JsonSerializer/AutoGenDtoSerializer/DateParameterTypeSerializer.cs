// -------------------------------------------------------------------------------------------------------------------------------
// <copyright file="DateParameterTypeSerializer.cs" company="Starion Group S.A.">
...
namespace CDP4JsonSerializer
{
    ...
    public class DateParameterTypeSerializer : BaseThingSerializer, IThingSerializer
    {
        ...
        /// <exception cref="ArgumentException">If the provided <paramref name="thing" /> is not an <see cref="DateParameterType" /></exception>
        /// <exception cref="NotSupportedException">If the provided <paramref name="requestedDataModelVersion" /> is not supported</exception>
        public void Serialize(Thing thing, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            if (thing is not DateParameterType dateParameterType)
            {
                throw new ArgumentException("The thing shall be a DateParameterType", nameof(thing));
            }

            if (requestedDataModelVersion < Version.Parse("1.0.0"))
            {
                Logger.Log(LogLevel.Info, "Skipping serialization of DateParameterType since Version is below 1.0.0");
            ...
            {
                case "1.0.0":
                    Logger.Log(LogLevel.Trace, "Serializing DateParameterType for Version 1.0.0");

                    //if (dateParameterType.Alias.Count > 0)
                    //{
                        writer.WriteStartArray("alias"u8);

                        foreach(var aliasItem in dateParameterType.Alias.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (dateParameterType.Category.Count > 0)
                    //{
                        writer.WriteStartArray("category"u8);

                        foreach(var categoryItem in dateParameterType.Category.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(dateParameterType.ClassKind.ToString());

                    //if (dateParameterType.Definition.Count > 0)
                    //{
                        writer.WriteStartArray("definition"u8);

                        foreach(var definitionItem in dateParameterType.Definition.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (dateParameterType.HyperLink.Count > 0)
                    //{
                        writer.WriteStartArray("hyperLink"u8);

                        foreach(var hyperLinkItem in dateParameterType.HyperLink.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(dateParameterType.Iid);
                    writer.WritePropertyName("isDeprecated"u8);
                    writer.WriteBooleanValue(dateParameterType.IsDeprecated);
                    writer.WritePropertyName("name"u8);
                    writer.WriteStringValue(dateParameterType.Name);
                    writer.WritePropertyName("revisionNumber"u8);
                    writer.WriteNumberValue(dateParameterType.RevisionNumber);
                    writer.WritePropertyName("shortName"u8);
                    writer.WriteStringValue(dateParameterType.ShortName);
                    writer.WritePropertyName("symbol"u8);
                    writer.WriteStringValue(dateParameterType.Symbol);
                    break;
                case "1.1.0":
                    Logger.Log(LogLevel.Trace, "Serializing DateParameterType for Version 1.1.0");
                    ...
                    //if (dateParameterType.ExcludedDomain.Count > 0)
                    //{
                        writer.WriteStartArray("excludedDomain"u8);

                        foreach(var excludedDomainItem in dateParameterType.ExcludedDomain.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (dateParameterType.ExcludedPerson.Count > 0)
                    //{
                        writer.WriteStartArray("excludedPerson"u8);

                        foreach(var excludedPersonItem in dateParameterType.ExcludedPerson.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(dateParameterType.ModifiedOn.ToString(SerializerHelper.DateTimeFormat));
                    writer.WritePropertyName("name"u8);
                    writer.WriteStringValue(dateParameterType.Name);
                    writer.WritePropertyName("revisionNumber"u8);
                    writer.WriteNumberValue(dateParameterType.RevisionNumber);
                    ...
                    break;
                case "1.2.0":
                    Logger.Log(LogLevel.Trace, "Serializing DateParameterType for Version 1.2.0");
                    ...
                    writer.WriteStringValue(dateParameterType.ThingPreference);
                    break;
                case "1.3.0":
                    Logger.Log(LogLevel.Trace, "Serializing DateParameterType for Version 1.3.0");
                    writer.WritePropertyName("actor"u8);

                    if(dateParameterType.Actor.HasValue)
                    {
                        writer.WriteStringValue(dateParameterType.Actor.Value);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Serialize a value for a <see cref="DateParameterType"/> property into a <see cref="Utf8JsonWriter" />
        /// </summary>
        ...
        public void SerializeProperty(string propertyName, object value, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            ...
            switch(propertyName.ToLower())
            {
                ...
                case "thingpreference":
                    ...
                    break;
                default:
                    throw new ArgumentException($"The requested property {propertyName} does not exist on the DateParameterType");
            }
        }
