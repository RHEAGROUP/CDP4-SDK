...
namespace CDP4JsonSerializer
{
    ...
    using ValueGroup = CDP4Common.DTO.ValueGroup;
    ...
    public class ValueGroupSerializer : BaseThingSerializer, IThingSerializer
    {
        ...
        /// <exception cref="ArgumentException">If the provided <paramref name="thing" /> is not an <see cref="ValueGroup" /></exception>
        /// <exception cref="NotSupportedException">If the provided <paramref name="requestedDataModelVersion" /> is not supported</exception>
        public void Serialize(Thing thing, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            if (thing is not ValueGroup valueGroup)
            {
                throw new ArgumentException("The thing shall be a ValueGroup", nameof(thing));
            }

            if (requestedDataModelVersion < Version.Parse("1.1.0"))
            {
                Logger.Log(LogLevel.Info, "Skipping serialization of ValueGroup since Version is below 1.1.0");
            ...
            {
                case "1.1.0":
                    Logger.Log(LogLevel.Trace, "Serializing ValueGroup for Version 1.1.0");

                    //if (valueGroup.Alias.Count > 0)
                    //{
                        writer.WriteStartArray("alias"u8);

                        foreach(var aliasItem in valueGroup.Alias.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (valueGroup.Category.Count > 0)
                    //{
                        writer.WriteStartArray("category"u8);

                        foreach(var categoryItem in valueGroup.Category.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(valueGroup.ClassKind.ToString());

                    //if (valueGroup.Definition.Count > 0)
                    //{
                        writer.WriteStartArray("definition"u8);

                        foreach(var definitionItem in valueGroup.Definition.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (valueGroup.ExcludedDomain.Count > 0)
                    //{
                        writer.WriteStartArray("excludedDomain"u8);

                        foreach(var excludedDomainItem in valueGroup.ExcludedDomain.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (valueGroup.ExcludedPerson.Count > 0)
                    //{
                        writer.WriteStartArray("excludedPerson"u8);

                        foreach(var excludedPersonItem in valueGroup.ExcludedPerson.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    //if (valueGroup.HyperLink.Count > 0)
                    //{
                        writer.WriteStartArray("hyperLink"u8);

                        foreach(var hyperLinkItem in valueGroup.HyperLink.OrderBy(x => x, this.GuidComparer))
                        {
                    ...
                    writer.WriteStringValue(valueGroup.Iid);
                    writer.WritePropertyName("modifiedOn"u8);
                    writer.WriteStringValue(valueGroup.ModifiedOn.ToString(SerializerHelper.DateTimeFormat));
                    writer.WritePropertyName("name"u8);
                    writer.WriteStringValue(valueGroup.Name);
                    writer.WritePropertyName("revisionNumber"u8);
                    writer.WriteNumberValue(valueGroup.RevisionNumber);
                    writer.WritePropertyName("shortName"u8);
                    writer.WriteStringValue(valueGroup.ShortName);
                    break;
                case "1.2.0":
                    Logger.Log(LogLevel.Trace, "Serializing ValueGroup for Version 1.2.0");
                    ...
                    writer.WriteStringValue(valueGroup.ThingPreference);
                    break;
                case "1.3.0":
                    Logger.Log(LogLevel.Trace, "Serializing ValueGroup for Version 1.3.0");
                    writer.WritePropertyName("actor"u8);

                    if(valueGroup.Actor.HasValue)
                    {
                        writer.WriteStringValue(valueGroup.Actor.Value);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Serialize a value for a <see cref="ValueGroup"/> property into a <see cref="Utf8JsonWriter" />
        /// </summary>
        ...
        public void SerializeProperty(string propertyName, object value, Utf8JsonWriter writer, Version requestedDataModelVersion)
        {
            ...
            switch(propertyName.ToLower())
            {
                ...
                case "thingpreference":
                    ...
                    break;
                default:
                    throw new ArgumentException($"The requested property {propertyName} does not exist on the ValueGroup");
            }
        }
